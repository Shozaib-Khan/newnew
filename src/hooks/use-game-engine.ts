...
// Clamp to max points and round to 1 decimal place
...
export const useGameEngine = () => {
  ...
  useEffect(() => {
    ...
    if (storedHighScore) setHighScore(parseFloat(storedHighScore));
  }, []);
  ...
  const isWall = useCallback(
    (pos: Position, mazeLayout: typeof maze) => {
  ...
  const resetCharactersToStart = useCallback(() => {
    ...
    setGhosts(
      GHOST_START_POSITIONS.map((pos, i) => ({
        ...pos,
        id: String(i),
        direction: 'up',
        isFrightened: false,
  ...
  const handleStartGame = useCallback(() => {
    const newMaze = INITIAL_MAZE_LAYOUT.map(row =>
      row.map(cell => {
    ...
    const pos = AARNA_COIN_FIXED_POSITIONS[Math.floor(Math.random() * AARNA_COIN_FIXED_POSITIONS.length)];
    newMaze[pos.x][pos.y].type = 9;

    let totalCoins = 0;
    newMaze.forEach(row => {
      row.forEach(cell => {
        if (cell.type === 2 || cell.type === 3 || cell.type === 9) totalCoins++;
      });
    });
  ...
  const handleSetDirection = useCallback(
    (dir: Direction) => {
      if (gameState === 'running') desiredDirectionRef.current = dir;
    },
    [gameState]
  );

  // Main game loop - ticks every GAME_SPEED ms
  // Player moves every PLAYER_SPEED_DIVISOR ticks
  // Ghosts move every GHOST_SPEED_MULTIPLIER ticks
  ...
  // Effect: handle coin eat, power pellet, aarna coin, ghost collisions
  useEffect(() => {
    ...
    if (cell.type === 2) {
      ...
      const now = Date.now();
      if (now - lastFrightenUpdateRef.current > 500) {
